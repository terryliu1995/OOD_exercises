...
class ElevatorQuestion {
	...
	class ElevatorButton {
		...
		public void pressButton()
		{
			...
			Elevator.getElevator().handleInternalRequest(request);
		}
	}
		...
		public void openGate() throws Exception
		{
			if(status == Status.UP)
			{
				for(int i = 0; i < upStops.size(); i++)
				{
					...
					{
						currLevel = checkLevel; //moveto currlevel
	...
	class ControlCenter{
		...
		public static boolean start() {
			while (!isEnd) {
				Elevator.getElevator().openGate();
				Elevator.getElevator().closeGate();
			}
		}
